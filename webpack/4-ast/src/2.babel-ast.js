 ...
 * 入参是两个：
 * code 字符串类型的源码
 * opts 选项可选  babelCore.TransformOptions 类型
 * 返回值：babelCore.BabelFileResult 类型
 * ts 真香
 * function transformSync(code: string, opts?: babelCore.TransformOptions): babelCore.BabelFileResult
 */
let targetCode = babelCore.transformSync(sourceCode, {
  // 使用的是箭头函数转换插件
